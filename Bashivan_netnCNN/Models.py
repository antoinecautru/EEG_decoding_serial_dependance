...
class MaxCNN(nn.Module):
    ...
    return x: output of the last layers after the log softmax
    '''
    def __init__(self, input_image=torch.zeros(1, 7, 3, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4, n_window=7):
    ...
    def forward(self, x):
        ...
        for i in range(self.n_window):
            tmp[:,i] = self.pool1( F.selu(self.conv7(self.pool1(F.selu(self.conv6(F.selu(self.conv5(self.pool1( F.selu(self.conv4(F.selu(self.conv3( F.selu(self.conv2(F.selu(self.conv1(x[:,i])))))))))))))))))
...
class TempCNN(nn.Module):
    ...
    def __init__(self, input_image=torch.zeros(1, 7, 3, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4, n_window=7):
        ...
        #Temporal CNN Layer
        # self.conv8 = nn.Conv1d(n_window,64,(4*4*128,3),stride=stride,padding=padding) # initial line
        self.conv8 = nn.Conv1d(in_channels=4*128*4, out_channels=64, kernel_size=3,stride=stride,padding=padding)
        
        self.pool = nn.MaxPool2d((n_window,1))
        self.drop = nn.Dropout(p=0.5)
        # self.fc = nn.Linear(64*3,n_classes)
        self.fc = nn.Linear(64*self.n_window,n_classes)
        self.max = nn.LogSoftmax()
    ...
    def forward(self, x):
        if x.get_device() == 0:
            ...
            tmp = torch.zeros(x.shape[0],x.shape[1],128,4,4).cpu()
        for i in range(self.n_window):
            tmp[:,i] = self.pool1( F.selu(self.conv7(self.pool1(F.selu(self.conv6(F.selu(self.conv5(self.pool1( F.selu(self.conv4(F.relu(self.conv3( F.selu(self.conv2(F.selu(self.conv1(x[:,i])))))))))))))))))
        # x = tmp.reshape(x.shape[0], x.shape[1],4*128*4,1) # initial line
        x = tmp.reshape(x.shape[0], x.shape[1], 4*128*4)
        x = x.transpose(1,2) # added to debug
...
class LSTM(nn.Module):
    ...
    return x: output of the last layers after the log softmax
    '''
    def __init__(self, input_image=torch.zeros(1, 7, 3, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4, n_units=128, n_window=7):
        ...
        # LSTM Layer
        # self.rnn = nn.RNN(input_size = 4*4*128, hidden_size = n_units, num_layers = n_window) # initial line
        self.rnn = nn.RNN(input_size = 4*4*128, hidden_size = n_units, num_layers = n_window, batch_first = True) # corrected line
        self.rnn_out = torch.zeros(2, 7, 128) # arbitrary. Even the shape will change 

        self.pool = nn.MaxPool2d((n_window,1))
        self.drop = nn.Dropout(p=0.5)
        self.fc = nn.Linear(128*self.n_window, n_classes)
        self.max = nn.LogSoftmax()
    ...
    def forward(self, x):
        ...
        for i in range(self.n_window):
            img = x[:, i]
            img = F.selu(self.conv1(img))
            img = F.selu(self.conv2(img))
            img = F.selu(self.conv3(img))
            img = F.selu(self.conv4(img))
            img = self.pool1(img)
            img = F.selu(self.conv5(img))
            img = F.selu(self.conv6(img))
            img = self.pool1(img)
            img = F.selu(self.conv7(img))
            tmp[:, i] = self.pool1(img)
            del img
        x = tmp.reshape(x.shape[0], x.shape[1], 4 * 128 * 4) #(batch_size, n_windows=7, 2048)
        del tmp
        self.rnn_out, _ = self.rnn(x)
        # x = self.rnn_out.view(x.shape[0], -1) # initial line
        x = self.rnn_out.reshape(x.shape[0], -1) # corrected line
        x = self.fc(x)
        x = self.max(x)
        return x
...
class Mix(nn.Module):
    ...
    def __init__(self, input_image=torch.zeros(1, 7, 3, 32, 32), kernel=(3,3), stride=1, padding=1,max_kernel=(2,2), n_classes=4, n_units=128, n_window=7):
        ...
        # LSTM Layer
        #self.rnn = nn.RNN(4*4*128, n_units, n_window) # initial line
        self.rnn = nn.RNN(input_size = 4*4*128, hidden_size = n_units, num_layers = n_window, batch_first = True) # corrected line
        self.rnn_out = torch.zeros(2, 7, 128)

        # Temporal CNN Layer
        #self.conv8 = nn.Conv1d(n_window, 64, (4 * 4 * 128, 3), stride=stride, padding=padding) # initial line
        self.conv8 = nn.Conv1d(in_channels=4*128*4, out_channels=64, kernel_size=3,stride=stride,padding=padding) # corrected line

        self.pool = nn.MaxPool2d((n_window, 1))
        self.drop = nn.Dropout(p=0.5)
        # self.fc1 = nn.Linear(1088,512) # initial line
        self.fc1 = nn.Linear(320 + 128*self.n_window,512) # corrected line
        self.fc2 = nn.Linear(512, n_classes)
        self.max = nn.LogSoftmax()
    ...
    def forward(self, x):
        ...
        # temp_conv = F.relu(self.conv8(tmp.reshape(x.shape[0], x.shape[1], 4 * 128 * 4, 1))) # initial line
        temp_conv = F.relu(self.conv8(tmp.reshape(x.shape[0], x.shape[1], 4 * 128 * 4).transpose(1,2))) # corrected line
        temp_conv = temp_conv.reshape(temp_conv.shape[0], -1)
        self.lstm_out, _ = self.rnn(tmp.reshape(x.shape[0], x.shape[1], 4 * 128 * 4))
        del tmp
        #lstm = self.lstm_out.view(x.shape[0], -1) # initial line
        lstm = self.lstm_out.reshape(x.shape[0], -1) # corrected line
        x = torch.cat((temp_conv, lstm), 1) # (batch_size, 1344)
