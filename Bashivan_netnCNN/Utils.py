...
def Test_Model(net, Testloader, criterion, is_cuda=False):
    running_loss = 0.0 
    evaluation = []
...
def TrainTest_Model(model, trainloader, testloader, n_epoch=30, opti='SGD', learning_rate=0.0001, is_cuda=False, print_epoch =5, verbose=False, n_classes=4, n_window=7):
    if is_cuda:
        net = model(n_classes=n_classes, n_window=n_window).cuda()
    else :
        net = model(n_classes=n_classes, n_window=n_window)
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            # print(inputs.to(torch.float32).squeeze(-1).size())
            outputs = net(inputs.to(torch.float32).squeeze(-1))
            _, predicted = torch.max(outputs.cpu().data, 1)
            evaluation.append((predicted==labels).tolist())
            #loss = criterion(outputs, labels.cuda())
            loss = criterion(outputs, labels.to(torch.long))
        ...
        evaluation = [item for sublist in evaluation for item in sublist]
        running_acc = sum(evaluation)/len(evaluation)
        validation_loss, validation_acc = Test_Model(net, testloader, criterion, is_cuda=False)
...
def create_img(PATH="Sample Data", Nelectrodes=64, participant=""):
    feats = sio.loadmat(f'{PATH}/{participant}_FeatureMat_timeWin.mat')['features']
    locs = sio.loadmat(f'{PATH}/Neuroscan_locs_orig.mat')
    locs_3d = locs['A']
    locs_2d = []
    # Convert to 2D
    for e in locs_3d:
    ...
    Window_size = Nelectrodes*3
    images_timewin = np.array([gen_images(np.array(locs_2d),
                                          feats[:, i * Window_size:(i + 1) * Window_size], 32, normalize=True) for i in
                               range(int(feats.shape[1] / Window_size))
                               ])

    sio.savemat(f"{PATH}/{participant}_images_time.mat",{"img":images_timewin})
    print(f"Images Created and Save in {PATH}/images_time")